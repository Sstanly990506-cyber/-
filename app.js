...
const APP_BUILD = "2026-03-01-hotfix-3";
...
function getOrderReceivableKey(order) {
  return (order.orderNumber || "").trim() || `ORDER-${order.id}`;
}

function syncOrderToReceivables(order) {
  const key = getOrderReceivableKey(order);
  const index = state.receivables.findIndex((r) => (r.orderNumber || "").trim() === key);
  ...
  if (index >= 0) state.receivables[index] = linked;
...
function save() {
  ...
  pushServerState(now);
  markSynced("已儲存");
  updateSyncDetail(fileModeOnly ? "本機儲存" : "集中式資料庫", now);
}
...
function getLinkedReceivablesData() {
...
function getReportAData() {
  return getLinkedReceivablesData();
}
