...
function renderInvoicePicker(state) {
  ...
  if (!orders.length) {
    wrap.innerHTML = '<p class="sub">目前沒有可開立發票的工單（請先將工單狀態設為已送出或已完成）。</p>';
  ...
  orders.forEach((order) => {
    ...
    const checked = selectedInvoiceOrderIds.has(order.id) ? 'checked' : '';
    const amount = Number(order.totalPrice || 0);
    row.innerHTML = `
      <input type="checkbox" data-invoice-order-id="${order.id}" ${checked} />
      <strong>${order.orderNumber || '未填工單號'}</strong>
      <span>${order.downstream || order.upstream || '-'}</span>
      <span>${order.orderDate || '-'}</span>
      <span>NT$ ${money(amount)}</span>
    `;
    wrap.append(row);
  });
...
function openInvoiceWindow(state) {
  ...
  const rows = selected
    .map(
      ...
      <tr>
        <td>${idx + 1}</td>
        <td>${o.orderNumber || '-'}</td>
        <td>${o.downstream || o.upstream || '-'}</td>
        <td>${o.orderDate || '-'}</td>
        <td style="text-align:right;">${money(Number(o.totalPrice || 0))}</td>
      </tr>`
    )
    .join('');
  ...
  <title>發票匯出</title>
  <style>
  body{font-family:"Noto Sans TC",sans-serif;padding:16px;color:#111}
  .copy{border:2px solid #555;padding:12px;margin-bottom:12px}
  h2{margin:0 0 8px}
  table{width:100%;border-collapse:collapse;font-size:13px}
  th,td{border:1px solid #777;padding:6px}
  .meta{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px}
  .sum{font-weight:700;text-align:right;margin-top:8px}
  </style></head>
  <body>
    <div class="copy"><h2>電子發票（白聯）</h2>${renderInvoiceMeta(meta)}<table><thead><tr><th>#</th><th>工單號</th><th>客戶</th><th>日期</th><th>金額</th></tr></thead><tbody>${rows}</tbody></table><div class="sum">合計：NT$ ${money(total)}</div></div>
    <div class="copy"><h2>電子發票（粉紅聯）</h2>${renderInvoiceMeta(meta)}<table><thead><tr><th>#</th><th>工單號</th><th>客戶</th><th>日期</th><th>金額</th></tr></thead><tbody>${rows}</tbody></table><div class="sum">合計：NT$ ${money(total)}</div></div>
...
export function renderFinance(state) {
  ...
  const isReports = state.financeScreen === 'reports';
  const isInvoice = state.financeScreen === 'invoice';
  $('financeMainScreen').classList.toggle('hidden', !isMain);
  $('financeReportsScreen').classList.toggle('hidden', !isReports);
  $('financeReportsTables').classList.toggle('hidden', !isReports);
  $('financeReportCWrap').classList.toggle('hidden', !isReports);
  $('financeInvoiceScreen').classList.toggle('hidden', !isInvoice);
